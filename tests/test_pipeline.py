"""End-to-end tests: physics parity, the training loop, evaluation.

Run on a few hundred generated voxels. They check that the pieces fit together and that the
training-side forward model matches the generator; they say nothing about model quality.
"""
...
def test_physics_parity_with_generator():
    """The training-side numpy forward must match the generator bit-for-bit."""
...
def test_physics_violations_are_measured_not_fixed():
    """Independent sigmoid heads can emit T2 >= T1; it is reported, not clamped."""
