"""Shape and gradient tests; nothing is trained.

A config loads, the dataset yields the tensors the model expects, a forward pass has the right
shape, and the Hungarian loss gives a finite scalar with a gradient.
"""
...
def test_config_roundtrip(tmp_path):
    ...
    p = tmp_path / "c.yaml"
    cfg.save(p)
    roundtrip = load_config(p)
    assert roundtrip.name == cfg.name
    assert roundtrip.loss.signal_consistency is True
...
def test_max_comp_is_not_configurable():
    """A config copy of the table width could go stale against the data, so there is none."""
...
def test_loader_rejects_n_comp_beyond_the_available_slots(tmp_path):
    """n_comp above the column width must raise.

    With a configured width this combination trained a model that could not count past the
    configured number while reporting plausible metrics: the cost matrix was built at the config
    width and numpy returns 2 columns when asked for 4.
    """
    import pytest
