...
This repository was written for the bachelor's thesis *Detection Transformer for
Microstructure Quantification from T1-T2 Correlation MRI* (Fatih Özkan; advisor Sebastian
Endt, Technische Hochschule Ingolstadt). It was written from scratch for T1-T2 data rather
than forked from any of the repositories below.

The Detection Transformer architecture itself comes from the work listed below. What the
thesis adds is the framing of the T1-T2 inverse problem as set prediction, an experiment
matrix that changes one thing at a time, and the finding that much of the trouble with small
compartments comes from the loss weighting rather than from the protocol.
...
treating microstructure quantification as DETR set prediction in the first place. An earlier
version of this code also carried his peak-grouping postprocessing, adapted to (T1, T2,
weight), and a set of figures in his style. Neither made it into the thesis, and both were
...
it, adapted to two feature dimensions and to log-space normalisation. One deliberate change:
he searches the existence threshold on the test split, and here it is calibrated on
validation instead.
