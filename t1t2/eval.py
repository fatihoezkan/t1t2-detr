"""Scoring a trained model: predictions in, metrics and figures out.

A prediction is a list of (T1_ms, T2_ms, weight) tuples per voxel, and so is the ground
truth. Pairing predictions with the truth is the same assignment problem the loss solves.
CSF (true T2 above CSF_T2_MS) is reported on its own line because this protocol barely
constrains its T2, so it should neither drag down nor flatter the headline numbers.
"""
...
# A pool with T2 above this is treated as CSF. The longest echo time in the protocol is 150 ms,
# so exp(-TE/T2) hardly moves for a 2000 ms pool and its T2 is close to unconstrained.
CSF_T2_MS = 1000.0


def detr_query_outputs(model, ds, device, normalizer, batch_size=512):
    """Return every query before thresholding, converted back to physical units.

    Returns a dict with `params`, an (N, Q, 3) array of T1 ms, T2 ms and weight, and
...
def predictions_from_query_outputs(query_outputs, exist_thresh=0.5):
    """Turn the unfiltered query table into the list-of-compartments format used for scoring."""
...
def detr_predictions(model, ds, device, normalizer, exist_thresh=0.5, batch_size=512):
    """Run the model and keep the queries whose existence score clears the threshold.

    Surviving queries have T1 and T2 converted back to milliseconds.
    """
...
def true_compartments(ds):
    """Read the ground-truth compartments out of a dataset in the same list format.
...
def _match(pred, true):
    """Pair predicted with true compartments by Hungarian assignment in (log T1, log T2) space.

    Log space, so a miss on a short-T2 pool counts as heavily as a proportionally similar miss
    on a long one. Returns an empty list if either side is empty.
    """
...
def parameter_recovery_analysis(preds, trues, weight_bins=None):
    """Score set recovery on the parameters rather than on the count.

    Reports the signal-fraction-weighted relative T1/T2 error over matched compartments, the
    fraction of true signal weight recovered (so a missed pool stays visible), the set-level L1
    weight error including unmatched compartments on both sides, and a bounded parameter-set
    error. The bounded score charges a missed true compartment a 100 % T1/T2 error, clips a
    matched relative error at 100 %, adds the set-level weight L1 clipped at one, and averages
    the three; its complement is parameter_set_accuracy. It exists to give the validation
    threshold search an interior optimum. The unbounded errors are the ones to interpret.
    """
...
def _regression_block(preds, trues, prefix=""):
    ...
    return {
        ...
        # Explicit names: both the median and the conventional mean absolute error.
...
def count_confusion(preds, trues, n_queries=10):
    """Confusion matrix of true compartment count (rows) against predicted count (columns).
...
def physics_violations(preds):
    """Rate of T2 >= T1 predictions and deviation of the per-voxel weight sum from one.
...
def scatter_figure(preds, trues, path, title=""):
    """Predicted against true values on log axes, one panel for T1 and one for T2.

    Each point is a matched pair; the identity line is drawn in. matplotlib is imported here
    on the Agg backend so the module imports on a headless cluster node.
    """
    import matplotlib
    matplotlib.use("Agg")
    ...
    if pairs:
        ...
        for k, (ip, it, name) in enumerate([(0, 1, "T1"), (2, 3, "T2")]):
            ...
            ax[k].set_aspect("equal", "box")                    # square panel, 45 degree diagonal
...
def threshold_calibration_figure(calibration, path):
    """Plot the validation threshold sweep and mark the selected threshold."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
...
def evaluate_snr_ladder(model, paths, cfg, device, normalizer, results_dir, train_snr_min=None,
    ...
    """Score each fixed-SNR rung separately.

    `paths` maps a rung label to the parquet path or paths for that rung. The rungs share the
    same voxels and the same standardised noise draw and differ only in amplitude, so
    differences between rungs are the SNR effect alone. Rungs below the training SNR range are
    flagged as extrapolation and should not be folded into the in-distribution numbers.
    """
...
def _snr_of(label):
    """Extract the SNR from a rung label such as 'test_snr20'."""
